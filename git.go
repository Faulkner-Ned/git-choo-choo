...
func validateBranchAndRemote(branch, remote string) error {
	...
	cmdLocal.Stdout = &outLocal
	cmdLocal.Stderr = &outLocal
...
func getCommitInformation(branch, remote string) ([]CommitInfo, error) {
	...
	// Make sence as you build on earlier commit, so final commit would be the last attached to the train
...
func pushToRemote(branch, remote string, force bool) error {
	if force {
		fmt.Println("Full speed ahead! I hope you know what you're doing.")
	}
